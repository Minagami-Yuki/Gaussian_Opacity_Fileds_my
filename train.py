...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # print(f"Image shape: {image.shape}, Ground truth shape: {gt_image.shape}")
        ...
        # print(f"Image device: {image.device}, Ground truth device: {gt_image.device}")
        # print(f"Image shape: {image.shape}, Ground truth shape: {gt_image.shape}")
        ...
        is_save_images = True
        if is_save_images and (iteration % 7000 == 0):
            with torch.no_grad():
                ...
                for cam_idx, eval_cam in enumerate(allCameras):
                    rendering = render(eval_cam, gaussians, pipe, background, kernel_size=dataset.kernel_size)["render"]
                    image = rendering[:3, :, :]
                    transformed_image = L1_loss_appearance(image, eval_cam.original_image.cuda(), gaussians, eval_cam.idx, return_transformed_image=True)
                    ...
                    torchvision.utils.save_image(image_to_show, f"{dataset.model_path}/log_images/{iteration}_{cam_idx}.jpg")
